...
export function DoctorKitPage() {
  ...
  return <div className="app-page"><SiteHeader /><main className="app-shell result-workspace result-workspace--compact">
    ...
    {screening && <section className="doctor-kit" aria-labelledby="doctor-kit-title">
      <div className="doctor-kit__intro"><p className="app-kicker">Doctor Kit</p><h1 id="doctor-kit-title">Siapkan diskusi dengan dokter</h1><p>Pilih pertanyaan yang ingin Anda bawa saat bertemu dokter. Anda dapat menyiapkan PDF bila diperlukan.</p></div>
      <section className="doctor-kit__summary"><p className="app-kicker">Ringkasan NAYANA</p><p>{summary?.general_information || summary?.overview || 'Model menemukan pola pada foto fundus yang perlu dipahami bersama keluhan, riwayat kesehatan, dan pemeriksaan langsung oleh dokter mata.'}</p><p className="doctor-kit__disclaimer">Bukan diagnosis medis. Konfirmasi dengan dokter mata.</p></section>
      <section className="doctor-kit__questions" aria-labelledby="doctor-kit-questions"><h2 id="doctor-kit-questions">Pertanyaan untuk dokter mata</h2>{discussionQuestionsFor(screening).map((item) => <label key={item.id}><input type="checkbox" checked={selected.includes(item.question)} onChange={() => setQuestions(selected.includes(item.question) ? selected.filter((question) => question !== item.question) : [...selected, item.question])} /><span>{item.question}</span></label>)}</section>
      <ScreeningSaveActions result={screening} summary={summary} initialDestination={savedDestination} onSaved={setSavedDestination} purpose="unlock-chat" />
      ...
      }}> <span><b>Tanya NAYANA</b><small>{savedDestination ? 'Buka ruang percakapan hasil ini' : 'Tersedia setelah hasil disimpan'}</small></span><i aria-hidden="true">›</i></button>
