...
def get_disease_detail(disease_name):
    ...
    if not provider.api_key:
        return (
            ...
            "Netra Runtime API key is not configured."
        )
    ...
    try:
        ...
        final_text = json.loads(response.text).get("answer", "").strip()
        if not final_text:
            return "Additional guidance is temporarily unavailable."
        return markdown2.markdown(final_text)
    except Exception:
        logger.exception("Netra Runtime guidance generation failed")
        return "Additional guidance is temporarily unavailable."
